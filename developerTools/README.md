...
現在開いているページでダウンロードされたHTML、CSS、JavaScript、画像などのリソースが一覧で確認できます。
...
また、リソースを選択した状態で右クリックで`Local Modifications`を選択することで変更の差分を確認できます。

### ワークスペースを利用してファイルに変更を保存

ワークスペースを使うことでデベロッパーツール上で変更した内容をローカルファイルにそのまま保存することができます。

リソース一覧の`Filesystem`タブを選択して、**Add folder to workspace**を選択します。
...
リソース一覧の`Snippets`タブを選択して**New snippet**で追加できます。
